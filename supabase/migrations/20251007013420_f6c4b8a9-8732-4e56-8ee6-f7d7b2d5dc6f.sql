-- Replace functions to remove references to dropped plaintext columns (u.email, u.phone)
-- and use only encrypted fields via public.decrypt_sensitive_data

-- 1) get_user_decrypted
...
BEGIN
  ...
  IF auth.uid() = target_user_id OR public.has_role(auth.uid(), 'admin') THEN
    ...
    SELECT 
      u.id,
      -- Use only encrypted columns (plaintext columns were removed)
      COALESCE(public.decrypt_sensitive_data(u.email_encrypted), '') as email,
      COALESCE(public.decrypt_sensitive_data(u.phone_encrypted), '') as phone,
