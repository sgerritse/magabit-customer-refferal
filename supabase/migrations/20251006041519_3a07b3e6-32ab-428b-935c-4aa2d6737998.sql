...
-- Remove duplicate policies and ensure proper access controls with audit logging

-- Drop all existing policies on stripe_customers
DROP POLICY IF EXISTS "Users can view own payment methods" ON public.stripe_customers;
DROP POLICY IF EXISTS "Users can view their own payment methods" ON public.stripe_customers;
DROP POLICY IF EXISTS "Users can insert own payment methods" ON public.stripe_customers;
DROP POLICY IF EXISTS "Users can update own payment methods" ON public.stripe_customers;
DROP POLICY IF EXISTS "Admins can view all payment methods" ON public.stripe_customers;
DROP POLICY IF EXISTS "Admins can view payment metadata" ON public.stripe_customers;
DROP POLICY IF EXISTS "Admins can update payment methods" ON public.stripe_customers;
...
-- Note: PostgreSQL doesn't support SELECT triggers, so we audit UPDATE/DELETE only
...
DO $$
BEGIN
  RAISE NOTICE 'Payment card data security updated: Duplicate policies removed, audit logging enforced for all admin modifications';
END $$;